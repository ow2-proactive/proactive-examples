...
MODEL_PATH = os.path.join(os.getcwd(), str(uuid.uuid4()) + ".zip")
...
append = variables.get("APPEND") \
    if variables.get("APPEND") else raiser("APPEND should be specified")
model_url = variables.get("MODEL_URL") \
    if variables.get("MODEL_URL") else None
api_deploy = variables.get("DEPLOY_ENDPOINT") \
    if variables.get("DEPLOY_ENDPOINT") else raiser("DEPLOY_ENDPOINT is None")
