...
if is_labeled_data:
  ...
  perm = PermutationImportance(loaded_model, random_state=1).fit(dataframe_test.values, dataframe_label.values.ravel())
  ...
  for i in partial_feature_find:
    pdp_feature = pdp.pdp_isolate(model=loaded_model, dataset=dataframe_test, model_features=feature_names, feature=i) # preg
    pdp_plot_feature = pdp.pdp_plot(pdp_feature, i)  # ver personalizavel o nome da coluna
  ...
  inter1 = pdp.pdp_interact(model=loaded_model, dataset=dataframe_test, model_features=feature_names, features=features_to_plot2d)
  partial_plot = pdp.pdp_interact_plot(pdp_interact_out=inter1, feature_names=features_to_plot2d) # plot_type='contour'  plot_type='grid'
  ...
  shap_plot = shap.force_plot(explainer.expected_value[1], shap_values[1], data_for_prediction, matplotlib=True, show=False)
                ...
                <meta charset="UTF-8">
                  ...
                  <body class="container">
                    <h1 style="color:#003050;">
                      <center>Machine Learning Model Explainability</center>
                    </h1>
                    ...
                    <br>
                      <br>
                        <br>
                          ...
                          <p>The values towards the top are the most important features, and those towards the bottom matter least. The first number in each row shows how much model performance decreased with a random shuffling (in this case, using "accuracy" as the performance metric). The number after the ± measures how performance varied from one-reshuffling to the next (after repeating the shuffling of the columns different times).</p>  
                    ...
                    {0}
                          ...
                          <p>While feature importance shows what variables most affect predictions, partial dependence plots show how a feature affects predictions. The y axis is interpreted as change in the prediction from what it would be predicted at the baseline or leftmost value. A blue shaded area indicates level of confidence. </p>
                    ...
                    {2}
                          ...
                          <p> SHAP values interpret the impact of having a certain value for a given feature in comparison to the prediction we'd make if that feature took some baseline value. We will look at SHAP values for a single row of the dataset (we chose row 1). Feature values causing increased predictions are in pink, and their visual size shows the magnitude of the feature's effect. Feature values decreasing the prediction are in blue. </p> 
                    {3}               
